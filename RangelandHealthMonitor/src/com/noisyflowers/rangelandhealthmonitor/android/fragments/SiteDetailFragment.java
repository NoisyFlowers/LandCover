...
public class SiteDetailFragment extends Fragment implements IHelp, OnClickListener {
	...
	private AutoCompleteTextView dominantWoodySpeciesTV, dominantNonwoodySpeciesTV;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		**/
		...
		dominantWoodySpeciesTV = (AutoCompleteTextView) rootView.findViewById(R.id.fragment_site_detail_edittext_dominant_woody_species);
        List<String> woodySpecies = RHMApplication.getInstance().getDatabaseAdapter().getWoodySpeciesList();
		ArrayAdapter<String> woodyAdapter = new ArrayAdapter<String>(getActivity(), android.R.layout.simple_dropdown_item_1line, woodySpecies);
		dominantWoodySpeciesTV.setAdapter(woodyAdapter);
		dominantNonwoodySpeciesTV = (AutoCompleteTextView) rootView.findViewById(R.id.fragment_site_detail_edittext_dominant_nonwoody_species);
        List<String> nonwoodySpecies = RHMApplication.getInstance().getDatabaseAdapter().getNonwoodySpeciesList();
		ArrayAdapter<String> nonwoodyAdapter = new ArrayAdapter<String>(getActivity(), android.R.layout.simple_dropdown_item_1line, nonwoodySpecies);
		dominantNonwoodySpeciesTV.setAdapter(nonwoodyAdapter);
		...
		if (((SiteDetailActivity)getActivity()).siteID != null) {
			...
			if (transect.dominantWoodySpecies != null) {
				dominantWoodySpeciesTV.setText(transect.dominantWoodySpecies);
				if (transect.isSubmitted()) { //TODO: Not the best way to prevent change to uploaded values; species may not have been uploaded
					dominantWoodySpeciesTV.setEnabled(false);
				}
			}
			if (transect.dominantNonwoodySpecies != null) {
				dominantNonwoodySpeciesTV.setText(transect.dominantNonwoodySpecies);
				if (transect.isSubmitted()) { //TODO: Not the best way to prevent change to uploaded values; species may not have been uploaded
					dominantNonwoodySpeciesTV.setEnabled(false);
				}
			}
		}
		**/
	...
	public void onPause() {
		if (((SiteDetailActivity)getActivity()).siteID != null) {
			for (Transect.Direction direction : Transect.Direction.values()) { 
			    Transect transect = RHMApplication.getInstance().getDatabaseAdapter().getTransect(((SiteDetailActivity)getActivity()).siteID, direction);
			    String woodySpecies = dominantWoodySpeciesTV.getText().toString();
			    String nonwoodySpecies = dominantNonwoodySpeciesTV.getText().toString();
			    if (!"".equals(woodySpecies) || "".equals(nonwoodySpecies)) {
			    	transect.dominantWoodySpecies = woodySpecies;
			    	transect.dominantNonwoodySpecies = nonwoodySpecies;
	...
	public void onClick(View v) {
		switch(v.getId()) {
			...
			case R.id.fragment_site_detail_button_transect_north: {
				...
				intent.putExtra(TransectActivity.TRANSECT_DIRECTION, Transect.Direction.NORTH.name());
